...
export class WindowManager {
  ...
  constructor() {
    ipcMain.on('renderer-ready-for-mode-change', (_event, newMode) => {
      if (
        (newMode !== 'window' && newMode !== 'pet')
        || newMode !== this.currentMode
      ) {
        return;
      }

      const transitionId = this.modeTransitionId;
      ...
      this.modeTransitionTimer = setTimeout(() => {
        ...
        // A newer mode change supersedes this delayed continuation.
        if (
          transitionId !== this.modeTransitionId
          || newMode !== this.currentMode
        ) {
          return;
        }
  ...
  setWindowMode(mode: 'window' | 'pet'): void {
    ...
    this.modeTransitionId += 1;
    if (this.modeTransitionTimer) {
      clearTimeout(this.modeTransitionTimer);
      this.modeTransitionTimer = null;
    }

    // Hover state belongs to the current rendered mode. Keeping it across a
    // transition can make the full-screen pet window consume every mouse event.
  ...
  private continueSetWindowModeWindow(): void {
    ...
    this.applyMousePassthrough(false);
  ...
  private setWindowModePet(): void {
    ...
    // Window mode receives mouse events. Restore passthrough before turning the
    // BrowserWindow into a transparent, always-on-top pet surface.
    this.applyMousePassthrough(true);
  ...
  private applyMousePassthrough(ignore: boolean): void {
    if (!this.window) return;
  ...
  updateComponentHover(componentId: string, isHovering: boolean): void {
    // Ignore stale hover-enter events from a previous pet render, but always
    // accept hover-leave so an old component cannot remain registered forever.
    ...
    // Forced passthrough controls the native window, but the hover set must
    // still stay current for when forced passthrough is disabled again.
    if (this.forceIgnoreMouse) return;

    if (this.window) {
      const shouldIgnore = this.hoveringComponents.size === 0;
      this.applyMousePassthrough(shouldIgnore);
      if (!shouldIgnore) {
        this.window.setFocusable(true);
      }
    }
  ...
  toggleForceIgnoreMouse(): void {
    // A delayed renderer event from pet mode must never make the normal window
    // ignore mouse input.
