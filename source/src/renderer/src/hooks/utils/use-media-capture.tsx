...
export function useMediaCapture() {
  ...
  const { consumeSaveNextScreenCapture } = useAttachments();
  ...
  const captureFrame = useCallback(
    async (
      ...
      try {
        ...
        const maxWidth = options.preserveNativeResolution
          ? 0
          : getImageMaxWidth();
        ...
        if (options.losslessTextCapture) {
  ...
  const captureAllMedia = useCallback(
    async () => {
      ...
      if (screenStream) {
        ...
        if (screenFrame) {
          images.push({
            ...
            data: screenFrame,
            mime_type: "image/png",
            persist: consumeSaveNextScreenCapture(),
          });
        }
      }
    ...
    [cameraStream, screenStream, captureFrame, consumeSaveNextScreenCapture],
  );
