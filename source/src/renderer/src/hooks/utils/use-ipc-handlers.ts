...
export function useIpcHandlers() {
  ...
  const { toggleManualCapture } = useVAD();
  ...
  const manualAudioToggleHandler = useCallback(() => {
    void toggleManualCapture();
  }, [toggleManualCapture]);

  const manualAudioKeyDownHandler = useCallback((event: KeyboardEvent) => {
  ...
  useEffect(() => {
    if (!window.electron?.ipcRenderer) {
      window.addEventListener("keydown", manualAudioKeyDownHandler);
      return () => window.removeEventListener("keydown", manualAudioKeyDownHandler);
    }

    window.electron.ipcRenderer.removeAllListeners("manual-audio-toggle");
    window.electron.ipcRenderer.on("manual-audio-toggle", manualAudioToggleHandler);
    ...
    micToggleHandler,
    manualAudioToggleHandler,
    manualAudioKeyDownHandler,
    interruptHandler,
    scrollToResizeHandler,
