  ...
  const handleWebSocketMessage = useCallback((message: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'user-input-transcription':
        ...
        if (message.text) {
          appendHumanMessage(message.text, message.attachments);
        }
        break;
      case 'manual-audio-transcription':
        if (message.segment_id) {
          handleManualTranscription(message.segment_id, message.text || '');
        }
        break;
      case 'error':
        toaster.create({
        ...
        break;
      case 'memory-notification': {
        const isError = message.level === 'error';
        toaster.create({
          id: `daily-child-event-${isError ? 'error' : 'success'}`,
          title: message.message,
          description: message.description,
          type: isError ? 'error' : 'success',
          duration: isError ? 5000 : 3000,
          meta: { source: 'daily-child-events' },
        });
        break;
      }
      case 'group-update':
      ...
      case 'backend-synth-complete':
        ...
        break;
      case 'rinne-outfit-catalog':
      case 'rinne-outfit-switch-result':
        // The outfit provider owns the transactional renderer refresh.
        break;
      case 'conversation-chain-end':
